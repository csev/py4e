...
if [ -f .chinese ] ; then
    ...
    cat [0-9]*.mkd | $MY_PYTHON verbatim.py | tee tmp.verbatim | pandoc -s -N -f markdown+definition_lists -t latex --toc --default-image-extension=eps -V fontsize:10pt -V documentclass:book -V mainfont='Noto Serif CJK SC' --template=template.latex -o tmp.tex
    pandoc -V mainfont='Noto Serif CJK SC' [A-Z][A-Z]*.mkd -o tmp.app.tex
else
    cat [0-9]*.mkd | $MY_PYTHON verbatim.py > tmp.verbatim 
...
$MY_PYTHON texpatch.py < tmp.sed > tmp.patch
