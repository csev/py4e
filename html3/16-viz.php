...
<p>So far we have been learning the Python language and then learning
how to use Python, the network, and databases to manipulate data.</p>
<p>In this chapter, we take a look at three complete applications that
...
solving a real-world problem.</p>
<p>Each of the applications is a ZIP file that you can download and
extract onto your computer and execute.</p>
<h2 id="building-a-openstreetmap-from-geocoded-data">Building a
OpenStreetMap from geocoded data</h2>
<p> </p>
<p>In this project, we are using the OpenStreetMap geocoding API to
clean up some user-entered geographic locations of university names and
then placing the data on an actual OpenStreetMap.</p>
<figure>
<img src="../images/openstreet-map.png" alt="An OpenStreetMap" />
<figcaption aria-hidden="true">An OpenStreetMap</figcaption>
</figure>
<p>To get started, download the application from:</p>
<p><a
href="http://www.py4e.com/code3/opengeo.zip">www.py4e.com/code3/opengeo.zip</a></p>
<p>The first problem to solve is that these geocoding APIs are
...
<p></p>
<p>In the first phase we take our input “survey” data in the file
...
<p>Run the <em>geoload.py</em> program. This program will read the input
...
database.</p>
<p>Here is a sample run after there is already some data in the
database:</p>
...
<p>The <em>geoload.py</em> program can be stopped at any time, and there
...
get your database to have all of the geocoded data for your input.</p>
<p>Once you have some data loaded into <em>geodata.sqlite</em>, you can
...
<p>This is a JavaScript variable that contains a list of lists. The
syntax for JavaScript list constants is very similar to Python, so the
syntax should be familiar to you.</p>
<p>Simply open <em>where.html</em> in a browser to see the locations.
...
JavaScript or developer console for your browser.</p>
<h2 id="visualizing-networks-and-interconnections">Visualizing networks
and interconnections</h2>
<p> </p>
<p>In this application, we will perform some of the functions of a
...
JavaScript visualization library <a href="http://d3js.org/"
class="uri">http://d3js.org/</a> to produce the visualization
output.</p>
<p>You can download and extract this application from:</p>
<p><a
href="http://www.py4e.com/code3/pagerank.zip">www.py4e.com/code3/pagerank.zip</a></p>
<figure>
...
spider.</p>
<p>If you want to dump the contents of the <em>spider.sqlite</em> file,
you can run <em>spdump.py</em> as follows:</p>
<pre><code>(5, None, 1.0, 3, &#39;http://www.dr-chuck.com/csev-blog&#39;)
(3, None, 1.0, 4, &#39;http://www.dr-chuck.com/dr-chuck/resume/speaking.htm&#39;)
(1, None, 1.0, 2, &#39;http://www.dr-chuck.com/csev-blog/&#39;)
(1, None, 1.0, 5, &#39;http://www.dr-chuck.com/dr-chuck/resume/index.htm&#39;)
4 rows.</code></pre>
<p>This shows the number of incoming links, the old page rank, the new
page rank, the id of the page, and the url of the page. The
<em>spdump.py</em> program only shows pages that have at least one
incoming link to them.</p>
<p>Once you have a few pages in the database, you can run page rank on
the pages using the <em>sprank.py</em> program. You simply tell it how
many page rank iterations to run.</p>
<pre><code>How many iterations:2
1 0.546848992536
2 0.226714939664
[(1, 0.559), (2, 0.659), (3, 0.985), (4, 2.135), (5, 0.659)]</code></pre>
<p>You can dump the database again to see that page rank has been
updated:</p>
...
4 rows.</code></pre>
<p>You can run <em>sprank.py</em> as many times as you like and it will
...
ranking programs all the time.</p>
...
<em>sprank.py</em> long enough that the page rank values converge.</p>
...
<p>You can view this data by opening the file <em>force.html</em> in
...
to find the URL that is represented by the node.</p>
<p>If you rerun the other utilities, rerun <em>spjson.py</em> and press
...
to take our analysis of email data to the next level.</p>
...
<p><a
href="https://www.py4e.com/code3/gmane.zip">https://www.py4e.com/code3/gmane.zip</a></p>
<p>We will be using data from a free email list archiving service called
<a href="http://www.gmane.org">http://www.gmane.org</a>. This service is
very popular with open source projects because it provides a nice
searchable archive of their email activity. They also have a very
liberal policy regarding accessing their data through their API. They
have no rate limits, but ask that you don’t overload their service and
take only the data you need. You can read gmane’s terms and conditions
at this page:</p>
<p><a
href="http://www.gmane.org/export.php">http://www.gmane.org/export.php</a></p>
<p><em>It is very important that you make use of the gmane.org data
...
this free service and ruin it for the rest of us.</em></p>
<p>When the Sakai email data was spidered using this software, it
...
process to catch up with more recent messages.</p>
...
the base url.</p>
<p>The <em>gmane.py</em> file operates as a responsible caching spider
in that it runs slowly and retrieves one mail message per second so as
to avoid getting throttled by gmane. It stores all of its data in a
database and can be interrupted and restarted as often as needed. It may
take many hours to pull all the data down. So you may need to restart
several times.</p>
<p>Here is a run of <em>gmane.py</em> retrieving the last five messages
of the Sakai developer list:</p>
...
<p>The program scans <em>content.sqlite</em> from one up to the first
...
formatted message.</p>
<p>Sometimes <strong>gmane.org</strong> is missing a message. Perhaps
...
fields blank and restart <em>gmane.py</em>. This will unstick the
spidering process and allow it to continue. These empty messages will be
ignored in the next phase of the process.</p>
<p>One nice thing is that once you have spidered all of the messages and
have them in <em>content.sqlite</em>, you can run <em>gmane.py</em>
again to get new messages as they are sent to the list.</p>
<p>The <em>content.sqlite</em> data is pretty raw, with an inefficient
data model, and not compressed. This is intentional as it allows you to
look at <em>content.sqlite</em> in the SQLite Manager to debug problems
with the spidering process. It would be a bad idea to run any queries
against this database, as they would be quite slow.</p>
<p>The second process is to run the program <em>gmodel.py</em>. This
program reads the raw data from <em>content.sqlite</em> and produces a
cleaned-up and well-modeled version of the data in the file
<em>index.sqlite</em>. This file will be much smaller (often 10X
smaller) than <em>content.sqlite</em> because it also compresses the
header and body text.</p>
<p>Each time <em>gmodel.py</em> runs it deletes and rebuilds
<em>index.sqlite</em>, allowing you to adjust its parameters and edit
the mapping tables in <em>content.sqlite</em> to tweak the data cleaning
process. This is a sample run of <em>gmodel.py</em>. It prints a line
...
nearly a Gigabyte of mail data.</p>
...
tasks.</p>
<p>Domain names are truncated to two levels for .com, .org, .edu, and
...
are also forced to lower case, and some of the <span class="citation"
data-cites="gmane.org">@gmane.org</span> address like the following</p>
...
<p>In the <em>mapping.sqlite</em> database there are two tables that
...
tracked together.</p>
<p>You can rerun the <em>gmodel.py</em> over and over as you look at the
...
<em>index.sqlite</em>. This is the file to use to do data analysis. With
this file, data analysis will be really quick.</p>
...
<em>gmane.py</em> or even <em>gmodel.py</em>. They are all working on
the same data, but <em>gbasic.py</em> is using the compressed and
normalized data in <em>index.sqlite</em>. If you have a lot of data to
...
really start to explore and visualize your data.</p>
...
using <em>gword.htm</em> to produce a word cloud similar to the one at
the beginning of this section.</p>
<p>A second visualization is produced by <em>gline.py</em>. It computes
email participation by organizations over time.</p>
...
<p>Its output is written to <em>gline.js</em> which is visualized using
<em>gline.htm</em>.</p>
<figure>
<img src="../images/mailorg.png"
alt="Sakai Mail Activity by Organization" />
<figcaption aria-hidden="true">Sakai Mail Activity by
